...
### 3. the calculation of team and league run scoring averages

In previous versions of the app, the calculation of each team's average runs scored and runs allowed per game, the league average runs per game, and the comparison of the team to the league, was done first using base R's `apply` family of functions, along with 
...
For this iteration, preparing the data for the app is now done using `tidyr::map()` and `purrr::map()`. 
