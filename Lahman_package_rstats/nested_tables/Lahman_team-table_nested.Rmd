...
LG_RPG <- Teams_lgyr %>%
  select(yearID, lgID, R = lgR, RA = lgRA, G = lgG, leagueRPG = lgRPG, leagueRAPG = lgRAPG)
...
This only differs in the level of nesting; instead of year and league, it's only year.
...
MLB_RPG <- Teams_lgyr %>%
  select(yearID, R = mlbR, RA = mlbRA, G = mlbG, leagueRPG = mlbRPG, leagueRAPG = mlbRAPG)
